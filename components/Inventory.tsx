...
const InventoryItemRow: React.FC<{
  ...
  onUpdateField: (id: string, field: string, value: number) => Promise<void>;
  onRemove: (id: string) => Promise<void>;
}> = ({ item, onUpdateField, onRemove }) => {
  ...
  const handleBlur = async (field: string, value: number, setter: (v: number) => void, originalValue: number) => {
      ...
      <div className="grid grid-cols-3 gap-3 flex-1">
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none"
          />
        </div>
        <div className="space-y-1">
...
const Inventory: React.FC<InventoryProps> = ({ inventory, bookings, onUpdate, venueId }) => {
  ...
  const updateItemField = async (id: string, field: string, value: number) => {
    try {
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        <div className="p-6">
          ...
          <form onSubmit={addItem} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-slate-50 p-5 rounded-2xl border border-slate-100">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 tracking-widest">Buy Price (₹)</label>
              <div className="relative">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 tracking-widest">Sell Price (₹)</label>
              <div className="relative">
                <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input
                  type="number"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 tracking-widest">Initial Stock</label>
              <div className="relative">
                <Package className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input
                  type="number"
            ...
            <div className="md:col-span-4 flex justify-end">
              <button
                ...
                className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 disabled:opacity-50"
              >
